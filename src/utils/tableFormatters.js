 ...
 */
export function generateManifestJson(snap) {
  ...
  for (const [key, value] of Object.entries(manifest)) {
     ...
     */
    const meta = WingSnap.METADATA[key] ?? {}
    /** @type {Row4Col} */
    const tableRow = {
      setting: key,
      val1: value,
      label: meta.labelText ?? 'N/A',
...
export function gen4ColJson(snap, section = 'mon', options = {}) {
  ...
  switch (section) {
    ...
    case 'talk':
      ...
      logger.debug(
        `Generaring 4 Column Talkback JSON - title: "${tableTitle}" - section: ${section}`
      )
      break
    default:
  ...
  for (const [K1, V1, V2] of flattened) {
    ...
    const meta = WingSnap.METADATA[metaKey]
    // const newRow = [K1, V1, V2, meta.labelText, meta.manualText]
    const newRow = {
      setting: K1,
      val1: V1,
      val2: V2,
